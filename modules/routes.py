...
from modules.modals import User_mgmt, Post, Repost, Timeline, Bookmark
from modules.forms import Signup, Login, UpdateProfile, post
from modules.functions import save_bg_picture, save_profile_picture, delete_old_images, save_post_picture
...
def account():
    ...
    reposts = Repost.query\
        .filter_by(user_id=current_user.id)\
        .order_by(desc(Repost.id))

    return render_template('account.html',profile=profile_pic,background=bg_pic,update=update,timeline=all_posts, reposts=reposts)
...
def delete_account(account_id):
    ...
    all_reposts = Repost.query.filter_by(user_id=current_user.id)
...
def viewProfile(account_id):
    ...
    return render_template('view_profile.html',profile=profile_pic,background=bg_pic,timeline=all_posts,user=get_user,reposts=reposts)
...
@app.route('/repost/<int:post_id>',methods=['GET','POST'])
@login_required
def repost(post_id):
    ...
    if new_tweet.validate_on_submit():
        ...
        to_timeline = Timeline(repost_id=repost.id)
        db.session.add(to_timeline)
        db.session.commit()

        msg = 'You reposted @'+post.author.username+"'s tweet!"
        flash(msg,'success')
        return redirect(url_for('dashboard'))

    return render_template('repost.html',post=post, tweet=new_tweet)
...
@app.route('/delete_repost/<int:post_id>')
@login_required
def delete_repost(post_id):
    repost = Repost.query.get_or_404(post_id)
    if repost.retwitter != current_user:
        abort(403)
    return render_template('delete_post.html',repost=repost)
...
@app.route('/delete_reposted_post/<int:post_id>',methods=['POST'])
@login_required
def delete_reposted_tweet(post_id):
    ...
    remove_from_timeline = Timeline.query.filter_by(repost_id=post_id).first()
    if remove_from_timeline.from_repost.retwitter != current_user:
        abort(403)
    db.session.delete(remove_from_timeline)
    db.session.commit()
